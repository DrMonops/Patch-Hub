...
function giveOre()
	...
	EndIf
	perk HighYieldPerk = (game.GetFormFromFile(96774, "Dawnguard.esm") as formlist).GetAt(3) as perk
	if !HighYieldPerk
		debug.Trace("VOKRII ERROR: GetFormFromFile in MineOreScript was unable to retrieve HighYieldPerk from the DLC1VampireSpellsPowers formlist. Was the formlist put into use and overwritten by another mod?", 2)
	endIf
	if ResourceCountCurrent > 0
...
; 			debug.Trace(self + ": ResourceCountCurrent == 0 - depleted" )
			...
			if ore
				if game.getPlayer().HasPerk(HighYieldPerk)
					game.getPlayer().addItem(Ore as Form, ResourceCount * 2, false)
				else
